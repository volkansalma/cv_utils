...
def get_qr_coords(cmtx, dist, points):
    ...
    # with respect to camera coorindate system.
    ret, rvec, tvec = cv.solvePnP(qr_edges, points, cmtx, dist)

    # Define unit xyz axes. These are then projected to camera view
    # using the rotation matrix and translation vector.
    unitv_points = np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0], [0, 0, 1]],
        ...
        if ret_qr:
            ...
            if len(axis_points) > 0:
                ...
                for p, c in zip(axis_points[1:], colors[:3]):
                    ...
                    # Sometimes qr detector will make a mistake and projected
                    # point will overflow integer value. We skip these cases.
                    if (origin[0] > 5*img.shape[1]) or \
                            (origin[1] > 5*img.shape[1]):
                        break
